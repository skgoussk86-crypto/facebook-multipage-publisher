...
export function getGeminiAnalysisErrorMessage(
  ...
  if (status === 409) {
    return "The media file must finish uploading and validation before the AI can analyze it.";
  }

  if (status === 413) {
    return "This media file is larger than the configured AI analysis limit.";
  }

  if (status === 429) {
    return "The local AI service is currently busy processing another media file. Please wait and try again.";
  }
  ...
  return "The AI service could not analyze this media file. Try again later.";
}
