...
async function runTests() {
  ...
  const configId1 = randomUUID();
  const configId2 = randomUUID();

  await prisma.appConfiguration.createMany({
    data: [
      {
        id: configId1,
        userId: testUserId,
        configurationName: 'Test Config',
        facebookAppId: 'app-123',
        encryptedAppSecret: 'dummy-secret',
